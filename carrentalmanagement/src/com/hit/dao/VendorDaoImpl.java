...
public class VendorDaoImpl  implements VendorDao{
	...
	public String registerVendor(VendorBean vendor) {
		...
		try {
			...
			pst = con.prepareStatement("select * from customer where customer_email=?");
		...
		if(rs.next()){
			...
			try {
				...
				if(k>0) //update successful
					status = "Registration Successful.<br> Your Customer id: "+vendor.getId()+"<br>Thanks For Registration";
			}
	...
	public List<VendorBean> getAllVendors() {
		...
		try {
			...
			while(rs.next()){
				VendorBean vendor = new VendorBean(rs.getString("vid"),rs.getString("vname"), rs.getString("vmob"), rs.getString("vemail"), rs.getString("address"), rs.getString("password"));
	...
	public boolean validatePassword(String vid, String password) {
		...
		ResultSet rs = null;
		try {
			pst = conn.prepareStatement("select * from customer where customer_id=? and password=?");
	...
	public String updateProfile(VendorBean vendor) {
		...
		try {
			ps = con.prepareStatement("update customer set customer_name=?,customer_phone=?,customer_email=?,customer_address=? where customer_id=?");
			
			ps.setString(1,vendor.getName());
			ps.setString(2,vendor.getMobile());
			ps.setString(3, vendor.getEmail());
			ps.setString(4, vendor.getAddress());
			ps.setString(5, vendor.getId());
	...
	public String changePassword(String vendorId, String oldPassword, String newPassword) {
		...
		try {
			...
			ps = con.prepareStatement("update customer set password = ? where customer_id=?");
	...
	public VendorBean getVendorDataById(String vendorId) {
		...
		try {
			...
			if(rs.next()){
				vendor = new VendorBean(rs.getString("vid"),rs.getString("vname"), rs.getString("vmob"), rs.getString("vemail"), rs.getString("address"), rs.getString("password"));
